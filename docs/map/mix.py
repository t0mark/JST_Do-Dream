...
def save_region_names_to_csv():
    """지역명을 CSV 파일로 저장"""
    output_file = 'skorea-mixed-2018-geo.json'
    csv_file = '../../region_names.csv'
    ...
    try:
        ...
        for feature in data['features']:
            props = feature['properties']
            area_name = props.get('name', '') or props.get('CTP_KOR_NM', '')
            if area_name:
                region_names.append(area_name)
        
        # CSV 파일로 저장
        with open(csv_file, 'w', encoding='utf-8', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['지역'])  # 헤더
            for region in region_names:
                writer.writerow([region])
        
        print(f"✅ 지역명 CSV 파일 생성 완료! {csv_file}")
        print(f"   총 {len(region_names)}개 지역명 저장됨")
